...
- (void)addLayerWithCGImage:(CGImageRef)image
  ...
  CGRect screenRegion = CGRectMake(offset.x, offset.y, imageRegion.size.width,
                                   imageRegion.size.height);
  CGRect drawRegion = CGRectMake(offset.x, offset.y, imageRegion.size.width,
                                 imageRegion.size.height);

  if (screenRegion.origin.x + screenRegion.size.width > documentSize.width)
    imageRegion.size.width = screenRegion.size.width =
        documentSize.width - screenRegion.origin.x;
  if (screenRegion.origin.y + screenRegion.size.height > documentSize.height)
    imageRegion.size.height = screenRegion.size.height =
        documentSize.height - screenRegion.origin.y;
  if (screenRegion.origin.x < 0) {
    imageRegion.origin.x = fabs(screenRegion.origin.x); // modif abs to fabs
    screenRegion.origin.x = 0;
    screenRegion.size.width = imageRegion.size.width =
        imageRegion.size.width - imageRegion.origin.x;
  }
  if (screenRegion.origin.y < 0) {
    imageRegion.origin.y = fabs(screenRegion.origin.y); // modif abs to fabs
    screenRegion.origin.y = 0;
    screenRegion.size.height = imageRegion.size.height =
        imageRegion.size.height - imageRegion.origin.y;
  }
    ...
    if ((documentSize.width == 0) || (documentSize.height == 0))
      @throw [NSException
          exceptionWithName:NSGenericException
                     reason:@"You must specify a non-zero documentSize before "
                            @"calling addLayer:"
                   userInfo:nil];

    if (flattenedContext == NULL) {
      flattenedContext = CGBitmapContextCreate(
          NULL, documentSize.width, documentSize.height, 8, 0,
          CGImageGetColorSpace(image),
          kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedLast);
      CGContextSetRGBFillColor(flattenedContext, 1, 1, 1, 1);
      CGContextFillRect(flattenedContext, CGRectMake(0, 0, documentSize.width,
                                                     documentSize.height));
    }
    drawRegion.origin.y =
        documentSize.height - (drawRegion.origin.y + drawRegion.size.height);
...
/* Generates an NSData object representing a PSD image. LayerData should contain
 an array of NSData objects representing the RGBA layer data (8 bits per
 component) and a width and height of size. flatData should contain the RGBA
 data of a single image made by flattening all of the layers. */
...
- (void)preprocess {
  ...
  if (flattenedData == nil) {
    if (flattenedContext) {
      ...
      flattenedData = CGImageGetData(
          i, CGRectMake(0, 0, documentSize.width, documentSize.height));
      CGImageRelease(i);
    }
  }
  if (flattenedContext) {
  ...
  if ((self.shouldFlipLayerData == NO) &&
    ...
    if (self.shouldFlipLayerData) {
      ...
      for (int x = 0; x < documentSize.width; x++) {
        for (int y = 0; y < documentSize.height / 2; y++) {
          ...
          int bottom_index =
              (x + (documentSize.height - y - 1) * documentSize.width) * 4;
          char saved;
...
- (NSData *)createPSDData {
  ...
  if ((layerChannelCount < 3) || ([layers count] == 0))
    @throw [NSException
        exceptionWithName:NSGenericException
                   reason:@"Please provide layer data, flattened data and set "
                          @"layer channel count to at least 3."
                 userInfo:nil];
  ...
  // -----------------------------------------------------------------------------------------------
  // write images resources section. This is used to store things like current
  // layer.
  NSMutableData *imageResources = [[NSMutableData alloc] init];

  /*
   // Naming the alpha channels isn't necessary, but here's how:
   // Apparently those bytes contain 2 pascal strings? I think the last one is
   zero chars. [imageResources appendBytes:&signature8BIM length:4];
   ...
   Byte nameBytes[20] =
   {0x0C,0x54,0x72,0x61,0x6E,0x73,0x70,0x61,0x72,0x65,0x6E,0x63,0x79,0x05,0x45,0x78,0x74,0x72,0x61,0x00};
   [imageResources appendBytes:&nameBytes length:20];
   */

  // write the resolutionInfo structure. Don't have the definition for this, so
  // we have to just paste in the right bytes.
  ...
  Byte resBytes[16] = {0x00, 0x48, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01,
  ...
  NSData *transparentRowData = [NSData dataWithBytesNoCopy:transparentRow
                                                    length:transparentRowSize
                                              freeWhenDone:NO];
  NSData *packedTransparentRowData =
      [transparentRowData packedBitsForRange:NSMakeRange(0, transparentRowSize)
                                        skip:4];

  // LAYER + MASK INFORMATION SECTION
  // -----------------------------------------------------------------------------------------------
  // layer and mask information section. contains basic data about each layer
  // (its mask, its channels, its layer effects, its annotations, transparency
  // layers, wtf tons of shit.) We need to actually create this.
  ...
  for (int layer = 0; layer < layerCount; layer++) {
    ...
    NSData *imageData = [[layers objectAtIndex:0] imageData];
    CGRect bounds = [(PSDLayer *)[layers objectAtIndex:0] rect];
    ...
    if (bounds.origin.x < 0 || bounds.origin.y < 0) {
      @throw [NSException
          exceptionWithName:@"LayerOutOfBounds"
                     reason:[NSString stringWithFormat:
                                          @"Layer %i's x or y origin is "
                                          @"negative, which is unsupported",
                                          layer]
                   userInfo:nil];
    }
    if (bounds.origin.x + bounds.size.width > documentSize.width ||
      ...
      @throw [NSException
          exceptionWithName:@"LayerOutOfBounds"
                     reason:[NSString stringWithFormat:
                                          @"Layer %i's bottom-right corner is "
    ...
    NSData *packedLeftOfShape =
        [transparentRowData packedBitsForRange:leftPackRange skip:4];
    NSRange rightPackRange = NSMakeRange(
        0, (int)(documentSize.width - bounds.origin.x - bounds.size.width) * 4);
    NSData *packedRightOfShape =
        [transparentRowData packedBitsForRange:rightPackRange skip:4];

    for (int channel = 0; channel < layerChannelCount; channel++) {
      NSMutableData *byteCounts = [[NSMutableData alloc]
          initWithCapacity:documentSize.height * layerChannelCount * 2];
      NSMutableData *scanlines = [[NSMutableData alloc] init];

      for (int row = 0; row < documentSize.height; row++) {
        // If it's above or below the shape's bounds, just write black with
        // 0-alpha
        if (row < (int)bounds.origin.y ||
            row >= (int)(bounds.origin.y + bounds.size.height)) {
          [byteCounts appendValue:[packedTransparentRowData length]
                       withLength:2];
          ...
          NSRange packRange = NSMakeRange(
              (row - (int)bounds.origin.y) * imageRowBytes + channel,
              imageRowBytes);
    ...
    for (int c = 0; c < 3; c++) {
      ...
      [layerInfo
          appendValue:[[layerChannels objectAtIndex:c + layer * 4] length]
           withLength:4];
    }
    ...
    [layerInfo appendValue:[[layerChannels objectAtIndex:3 + layer * 4] length]
                withLength:4];
    ...
    // print out opacity
    int opacity = ceilf([[layers objectAtIndex:0] opacity] * 255.0f);
    ...
    [layerInfo appendValue:0 withLength:4];
    [layerInfo appendValue:0 withLength:4];
    //		char layerName[15] = {'L','a','y','e','r','s',' ','P','S','D','
    //','1','2','3','4'}; 		[layerInfo appendValue:15 withLength:1]; 		[layerInfo
    //appendBytes:&layerName length:15];

    //		NSString *layerNameString = [[layerNames objectAtIndex:layer]
    //stringByAppendingString:@" "]; 		[layerNameString getCString:layerName
    //maxLength:15 encoding:NSStringEncodingConversionAllowLossy];

    NSString *layerName =
        [[[layers objectAtIndex:0] name] stringByAppendingString:@" "];
    layerName = [layerName stringByPaddingToLength:15
                                        withString:@" "
                                   startingAtIndex:0];
    const char *layerNameCString =
        [layerName cStringUsingEncoding:NSStringEncodingConversionAllowLossy];
  ...
  // With RLE compression, the image data starts with the byte counts for all of
  // the scan lines (rows * channels) with each count stored as a 2-byte value.
  // The RLE compressed data follows with each scan line compressed separately.
  // Same as the TIFF standard.

  // in 512x512 image w/ no alpha, there are 3072 scan line bytes. At 2 bytes
  // each, that means 1536 byte counts. 1536 = 512 rows * three channels.

  NSMutableData *byteCounts = [NSMutableData
      dataWithCapacity:documentSize.height * layerChannelCount * 2];
  ...
  for (int channel = 0; channel < layerChannelCount; channel++) {
    ...
    for (int row = 0; row < documentSize.height; row++) {
      NSRange packRange =
          NSMakeRange(row * imageRowBytes + channel, imageRowBytes);
...
NSData *CGImageGetData(CGImageRef image, CGRect region) {
  ...
  context =
      CGBitmapContextCreate(bitmapData, width, height, 8, bitmapBytesPerRow,
  ...
  CGContextTranslateCTM(
      context, -region.origin.x + (drawRegion.size.width - region.size.width),
