...
#define MAATTACHEDWINDOW_DEFAULT_BACKGROUND_COLOR [NSColor colorWithCalibratedRed:76.00/255.00 green:161.00/255.00 blue:175.00/255.00 alpha:0.9]
#define MAATTACHEDWINDOW_DEFAULT_BORDER_COLOR [NSColor colorWithCalibratedRed:196.00/255.00 green:224.00/255.00 blue:229.0/255.00 alpha:0.8]
...
- (MAAttachedWindow *)initWithView:(NSView *)view attachedToPoint:(NSPoint)point inWindow:(NSWindow *)window onSide:(MAWindowPosition)side atDistance:(float)distance {
    ...
    int displayRetina = (int)[[NSScreen mainScreen] backingScaleFactor];
    
    if (displayRetina == 1) {
        
        MAATTACHEDWINDOW_SCALE_FACTOR = [[NSScreen mainScreen] backingScaleFactor];
    }
    else {
        
        MAATTACHEDWINDOW_SCALE_FACTOR = [[NSScreen mainScreen] backingScaleFactor] / 2;
    }
...
- (MAAttachedWindow *)initWithView:(NSView *)view attachedToPoint:(NSPoint)point inWindow:(NSWindow *)window atDistance:(float)distance {
...
- (MAAttachedWindow *)initWithView:(NSView *)view attachedToPoint:(NSPoint)point onSide:(MAWindowPosition)side atDistance:(float)distance {
...
- (MAAttachedWindow *)initWithView:(NSView *)view attachedToPoint:(NSPoint)point atDistance:(float)distance {
...
- (MAAttachedWindow *)initWithView:(NSView *)view attachedToPoint:(NSPoint)point inWindow:(NSWindow *)window {
...
- (void)_updateGeometry {
   ...
   // contentRect.origin = (_window) ? [_window convertBaseToScreen:_point] : _point;
    ...
    NSRect rct = [self getWindowRect]; // aici
    contentRect.origin = (_window) ? [_window convertRectToScreen:rct].origin : _point;  // aici
...
- (NSRect)getWindowRect { // aici
    
    NSRect aRect = NSMakeRect(_window.frame.origin.x, _window.frame.origin.y, _window.frame.size.width, _window.frame.size.height);
...
- (MAWindowPosition)_bestSideForAutomaticPosition {
  ...
  //  NSPoint pointOnScreen = (_window) ? [_window convertBaseToScreen:_point] : _point;
    ...
    NSRect rct = [self getWindowRect]; // aici
    NSPoint pointOnScreen = (_window) ? [_window convertRectToScreen:rct].origin : _point;  // aici
...
- (NSBezierPath *)_backgroundPath {
    ...
    if (scaledRadius > 0 && (drawsRoundCornerBesideArrow || (_side != MAPositionBottomRight && _side != MAPositionRightBottom))) {
    ...
    if (scaledRadius > 0 && (drawsRoundCornerBesideArrow || (_side != MAPositionBottomLeft && _side != MAPositionLeftBottom))) {
    ...
    if (scaledRadius > 0 && (drawsRoundCornerBesideArrow || (_side != MAPositionTopLeft && _side != MAPositionLeftTop))) {
    ...
    if (scaledRadius > 0 && (drawsRoundCornerBesideArrow || (_side != MAPositionTopRight && _side != MAPositionRightTop))) {
    ...
    if (scaledRadius > 0 && (drawsRoundCornerBesideArrow || (_side != MAPositionRightBottom && _side != MAPositionBottomRight))) {
...
- (void)setArrowBaseWidth:(float)value {
    ...
    float maxWidth = (MIN(_viewFrame.size.width, _viewFrame.size.height) +  (viewMargin * 2.0)) - cornerRadius;
...
- (void)setCornerRadius:(float)value {
    ...
    float maxRadius = ((MIN(_viewFrame.size.width, _viewFrame.size.height) +  (viewMargin * 2.0)) - arrowBaseWidth) / 2.0;
...
- (void)windowDidResignKey:(NSNotification *)aNotification {
    ...
    appDelegateObject.popupShowed = NO;
}
